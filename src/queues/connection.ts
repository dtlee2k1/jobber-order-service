...
const logger = winstonLogger(`${envConfig.ELASTIC_SEARCH_URL}`, 'orderQueueConnection', 'debug');
...
export async function createConnection() {
  try {
    ...
    logger.log({ level: 'error', message: `OrderService createConnection() method error: ${error}` });
    return undefined;
  }
}
